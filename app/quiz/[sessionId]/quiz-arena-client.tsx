...
export function QuizArenaClient({ sessionId, userName, initialState }: QuizArenaProps) {
  ...
  const getInitialState = (key: string, fallback: number) => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(`quiz_${sessionId}_${key}`);
      if (saved) return parseInt(saved, 10);
    }
    return fallback;
  };

  const [currentIndex, setCurrentIndex] = useState(() => getInitialState('index', 0));
  const [score, setScore] = useState(() => getInitialState('score', 0));
  
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(`quiz_${sessionId}_index`, currentIndex.toString());
      localStorage.setItem(`quiz_${sessionId}_score`, score.toString());
    }
  ...
  const processAnswer = async (optionIndex: number | null) => {
    ...
    if (isCorrect) {
      ...
      const speedBonus = Math.max(0, Math.floor(((question.timeLimit - timeSpent) / question.timeLimit) * 20));
    ...
    setTimeout(async () => {
      ...
      if (nextIndex >= allQuestions.length) {
        ...
        try {
          ...
          if (typeof window !== 'undefined') {
            localStorage.removeItem(`quiz_${sessionId}_index`);
            localStorage.removeItem(`quiz_${sessionId}_score`);
          }
          triggerCelebration();
          setIsCompleted(true);
        } catch (error) {
  ...
  if (isCompleted) {
    return (
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-serif font-bold mb-2">Quest Complete!</h1>
        <p className="text-xl text-white/70 mb-8">Great job, {userName}. XP Awarded.</p>
        
        <div className="bg-black/50 border border-white/10 rounded-3xl p-8 mb-8 w-full max-w-sm backdrop-blur-md relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center">
            <div className="text-sm font-bold text-white/50 uppercase mb-2">Total Score</div>
  ...
  // Loading State
  if (!question) {
    return <div className="flex items-center justify-center min-h-[50vh]"><Loader2 className="w-8 h-8 animate-spin text-brand" /></div>;
  }
    ...
    <div className="w-full max-w-4xl mx-auto flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-8 px-2">
        <div className="flex items-center gap-3">
          ...
          {/* Heatmap indicators could go here, but omitted for brevity. */}
        </div>
        
        <div className="flex items-center gap-2 bg-black/50 border border-white/10 rounded-full px-5 py-2 backdrop-blur-md">
          <span className="text-xs font-bold text-brand-light uppercase tracking-widest">Score</span>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={question.id} className="w-full">
          <Question 
            id={question.id}
            text={question.text}
            codeSnippet={undefined} // We could parse markdown or pass a specific field if added
            theme="dark"
            animationStyle="slide"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              {question.options.map((option: string, index: number) => {
                // Determine the status of this specific option card
                ...
                if (isSubmitting) {
                  ...
                  } else if (isSubmitting && validationStatus === 'incorrect' && index === question.correct_option_index) {
