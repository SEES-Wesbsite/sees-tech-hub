...
export function EventsClient({
  ...
  const [selectedClaimEvent, setSelectedClaimEvent] = useState<any | null>(
    null,
  );
      ...
      <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border/50">
        <div className="flex -space-x-2">
          {avatars.map((url, i) => (
            <div
              key={i}
              className="w-6 h-6 rounded-full border-2 border-background overflow-hidden relative bg-muted"
            >
              {url ? (
            ...
            {liveEvents.map((evt) => (
              ...
              >
                ...
                <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center bg-background/50 backdrop-blur-sm">
                  <Badge className="w-fit bg-success text-success-foreground mb-4">
                    HAPPENING NOW
                  </Badge>
                  ...
                  {evt.isClaimed ? (
                    <div className="bg-success/10 border border-success/30 rounded-xl p-4 flex items-center justify-center gap-3 text-success">
                      ...
                      <span className="font-bold">
                        Points Claimed! (+{evt.points_awarded} XP)
                      </span>
                    </div>
                  ) : (
                    <Button
              ...
              >
                ...
                <div className="w-full h-32 relative bg-muted">
                  ...
                  <div className="absolute bottom-3 left-4 flex gap-2">
                    <Badge
                      variant="secondary"
                      className="bg-background/80 backdrop-blur font-mono text-brand font-bold"
                    >
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <h3 className="text-xl font-bold font-serif text-foreground mb-2 line-clamp-2">
                    {evt.title}
                  </h3>
                  ...
                  {evt.description && (
                    <div className="mb-4 text-muted-foreground opacity-80 line-clamp-2 text-sm">
                      <MarkdownViewer
                        content={evt.description}
                        className="prose-sm dark:prose-invert text-sm"
                      />
                    </div>
                  )}
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
                  {pastEvents.map((evt) => (
                    <div
                      key={evt.id}
                      className="bg-foreground/5 rounded-2xl p-5 border border-border/50 opacity-70 grayscale hover:grayscale-0 transition-all"
                    >
