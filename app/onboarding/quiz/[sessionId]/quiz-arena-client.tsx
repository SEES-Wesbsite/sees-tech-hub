...
export function QuizArenaClient({ sessionId, userName, initialState }: QuizArenaProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(`onboarding_quiz_${sessionId}_index`, currentIndex.toString());
      localStorage.setItem(`onboarding_quiz_${sessionId}_score`, score.toString());
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLocalTime((prev: number) => {
        ...
        if (prev === 10) {
          toast(`Clock is ticking, ${userName}!`, { icon: '⏳', style: { background: '#1a1a1a', border: '1px solid #333' } });
        }
  ...
  if (isCompleted) {
    ...
    const handleShare = async () => {
      setIsSharing(true);
      const success = await captureAndShare(rankCardRef.current, `SEES-Rank-${userName}.png`);
      if (success) {
    ...
    return (
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-serif font-bold mb-2">Run Complete!</h1>
        <p className="text-xl text-white/70 mb-8">You survived the Arena, {userName}.</p>
        ...
        <div className="bg-black/50 border border-white/10 rounded-3xl p-8 mb-8 w-full max-w-md backdrop-blur-md relative overflow-hidden">
          ...
          <div className="relative z-10 grid grid-cols-2 gap-4">
            <div className="text-center">
              ...
              <div className="text-4xl font-black font-mono text-white">{score}</div>
            </div>
            <div className="text-center border-l border-white/10">
              <div className="text-sm font-bold text-white/50 uppercase mb-2">Final Rank</div>
              <div className="text-4xl font-black font-serif text-brand-light">{rankLetter}-Rank</div>
            </div>
          </div>
        </div>
